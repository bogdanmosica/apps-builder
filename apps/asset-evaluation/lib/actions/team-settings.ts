  ...
  try {
    ...
    const existingMember = teamData.team.teamMembers.find(
      (member: TeamMemberWithUser) =>
        member.user.email.toLowerCase() === validatedData.email,
    );
...
export async function updateMemberRole(formData: FormData) {
  try {
    ...
    const admins = teamData.team.teamMembers.filter(
      (member: TeamMemberWithUser) =>
        member.role === "admin" || member.role === "owner",
    );

    const targetMember = teamData.team.teamMembers.find(
      (member: TeamMemberWithUser) => member.id === validatedData.memberId,
    );
...
export async function removeMember(formData: FormData) {
  try {
    ...
    const targetMember = teamData.team.teamMembers.find(
      (member: TeamMemberWithUser) => member.id === memberId,
    );
    ...
    if (targetMember.userId === user.id && admins.length === 1) {
      return {
        success: false,
        message: "Cannot remove yourself - you are the only admin",
      };
    }
