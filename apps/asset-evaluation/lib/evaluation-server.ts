...
export async function saveEvaluationResults(
  ...
  try {
    console.log('💾 Starting evaluation save for user:', userId, 'property type:', propertyTypeId);
    ...
    if (process.env.NODE_ENV === 'production' || process.env.USE_NEON === 'true') {
      ...
      if (userAnswers.length > 0) {
        ...
        try {
          ...
          console.error('❌ Failed to insert answers, but session exists:', answerError);
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('relation') || error.message.includes('does not exist')) {
        throw new Error('Database schema error. Please ensure all tables exist.');
      }
      if (error.message.includes('foreign key constraint')) {
        throw new Error('Data integrity error. Please check that all referenced data exists.');
      }
    }
