...
describe("Translation Namespace Files Structure", () => {
  ...
  function loadTranslationFile(filePath) {
    try {
      ...
      throw new Error(
        `Failed to load translation file ${filePath}: ${error.message}`,
      );
    }
  }
  ...
  namespaces.forEach((namespace) => {
    describe(`${namespace} namespace`, () => {
      ...
      test("Romanian should have all English keys", () => {
        const data = allNamespaces[namespace];
        const missingKeys = data.englishKeys.filter(
          (key) => !data.romanianKeys.includes(key),
        );
      ...
      test("Romanian should not have extra keys", () => {
        const data = allNamespaces[namespace];
        const extraKeys = data.romanianKeys.filter(
          (key) => !data.englishKeys.includes(key),
        );
      ...
      test("key structures should be identical", () => {
        const data = allNamespaces[namespace];
        expect(JSON.stringify(data.romanianKeys)).toBe(
          JSON.stringify(data.englishKeys),
        );
      });
  ...
  test("translation namespace files summary", () => {
    ...
    console.log(
      `   ✨ Structure match: ${totalEnglishKeys === totalRomanianKeys ? "PERFECT" : "MISMATCH"}`,
    );
