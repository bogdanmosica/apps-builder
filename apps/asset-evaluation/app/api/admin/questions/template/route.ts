...
async function generateExcelTemplate(propertyTypeId: string | null, type: string) {
  ...
  let allPropertyTypes = await db.select().from(propertyTypes);

  if (propertyTypeId) {
    selectedPropertyType = allPropertyTypes.find((pt: any) => pt.id === parseInt(propertyTypeId));
  }
  ...
  // Instructions row
  const instructions = [
    selectedPropertyType ? selectedPropertyType.id.toString() : 'Property Type ID (required)',
  ...
  if (selectedPropertyType) {
    const existingData = await getExistingDataForPropertyType(selectedPropertyType.id);
  ...
  const instructionsSheet = createInstructionsSheet(allPropertyTypes, selectedPropertyType);
...
async function generateMarkdownTemplate(propertyTypeId: string | null, type: string) {
  let selectedPropertyType = null;
  let allPropertyTypes = await db.select().from(propertyTypes);
...
async function generateDataExport(propertyTypeId: string) {
  const exportData = await getExistingDataForPropertyType(parseInt(propertyTypeId));
...
function createInstructionsSheet(allPropertyTypes: any[], selectedPropertyType: any) {
  const instructions = [
    ...
    [''],
    ['AVAILABLE PROPERTY TYPES:'],
    ...allPropertyTypes.map(pt => [`ID: ${pt.id}`, pt.name_ro, pt.name_en || '']),
    [''],
    ['SMART LINKING (ID = 0):'],
