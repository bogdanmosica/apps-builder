...
import { evaluationSessions, users, propertyTypes } from '@/lib/db/schema';
...
export async function GET() {
  try {
    ...
    console.log('Session check:', session ? 'authenticated' : 'not authenticated', session?.user?.id);
    ...
    try {
      ...
      const userCheck = await db.select().from(users).where({ id: session.user.id } as any).limit(1);
    ...
    try {
      ...
      const [inserted] = await db.insert(evaluationSessions).values(testData).returning({ id: evaluationSessions.id });
