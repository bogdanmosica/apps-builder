...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      "📝 Session check:",
      session?.user ? `User ${session.user.id} authenticated` : "No session",
    );
    ...
    if (propertyTypesData.length === 0) {
      databaseStatus.recommendations.push(
        "❌ No property types found. Run seed script to populate the database.",
      );
    }

    if (questionsData.length === 0) {
      databaseStatus.recommendations.push(
        "❌ No questions found. Run seed script to populate the database.",
      );
    }

    if (usersData.length === 0) {
      databaseStatus.recommendations.push(
        "❌ No users found. Run seed script to create admin user.",
      );
    }

    if (!session?.user) {
      databaseStatus.recommendations.push(
        "⚠️ No authenticated user. Login required for evaluation saving.",
      );
    }

    if (databaseStatus.recommendations.length === 0) {
      databaseStatus.recommendations.push(
        "✅ Database appears to be properly configured and seeded.",
      );
    }
