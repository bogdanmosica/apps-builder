...
export async function GET(request: NextRequest) {
  try {
    ...
    const status = {
      ...
      buildInfo: {
        buildTime: process.env.VERCEL_GIT_COMMIT_SHA ? 'Vercel Build' : 'Local Build',
    ...
    if (propertyTypesData.length === 0) {
      status.recommendations.push('❌ Database not seeded. Property types missing.');
      status.recommendations.push('💡 Run: npm run db:seed or check Vercel build logs.');
      status.recommendations.push('🔧 Ensure DATABASE_URL is set in Vercel environment variables.');
    }
    
    if (usersData.length === 0) {
      status.recommendations.push('❌ No users in database. Run seed script to create admin user.');
    }
    
    if (!session) {
      status.recommendations.push('⚠️ No active session. Login required to save evaluations.');
    }
    
    if (!environment.hasDbUrl && !environment.hasPostgresUrl) {
      status.recommendations.push('❌ No database URL configured (DATABASE_URL or POSTGRES_URL).');
    }
    ...
    if (status.recommendations.length === 0) {
      status.recommendations.push('✅ All systems operational. Ready to save evaluations!');
    }
