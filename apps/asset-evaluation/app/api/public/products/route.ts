...
export async function GET() {
  try {
    ...
    const formattedProducts = activeProducts.map(
      (product: PublicProductData, index: number) => ({
        ...product,
        ...
        isPopular:
          ...
          index === Math.floor(activeProducts.length / 2),
      }),
    );
...
function generateFeaturesForProduct(name: string, price: number): string[] {
  ...
  const baseFeatures = ["SSL certificate", "Email support", "Basic analytics"];
  ...
  if (price < 20) {
    return [`Up to 5 ${baseName} projects`, ...baseFeatures, "5GB storage"];
  } else if (price < 50) {
    return [
