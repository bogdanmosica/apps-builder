...
async function handleDataExport(propertyTypeId: string | null) {
  ...
  const allPropertyTypes = propertyTypeId
    ? await db
        .select()
        .from(propertyTypes)
        .where(eq(propertyTypes.id, parseInt(propertyTypeId)))
...
async function getExistingQuestionsForTemplate(): Promise<
...
async function handleTemplateGeneration() {
  ...
  const propertyTypesList =
    existingPropertyTypes.length > 0
      ? existingPropertyTypes
          .map((pt: any) => `ID: ${pt.id} - ${pt.name_ro}`)
          .join(", ")
  ...
  if (existingQuestions.length > 0) {
    ...
    const samplePropertyTypeId =
      existingPropertyTypes.length > 0 ? existingPropertyTypes[0].id : 1;
