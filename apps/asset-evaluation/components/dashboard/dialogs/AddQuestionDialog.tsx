...
export default function AddQuestionDialog({
  ...
  const handleAnswerChange =
    (answerId: string, field: keyof Omit<AnswerFormData, "id">) =>
  ...
  const addAnswer = () => {
    const newId = (
      Math.max(...answers.map((a) => parseInt(a.id))) + 1
    ).toString();
    setAnswers((prev) => [
      ...prev,
      { id: newId, text_ro: "", text_en: "", weight: 0 },
    ]);
  };
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new question for the category: {category?.name_en} /{" "}
              {category?.name_ro}
            </DialogDescription>
          </DialogHeader>
