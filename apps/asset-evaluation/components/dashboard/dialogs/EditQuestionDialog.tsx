...
export default function EditQuestionDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const invalidAnswers = activeAnswers.some(
      (answer) =>
      ...
      const response = await fetch(`/api/admin/questions/${question.id}`, {
        ...
        body: JSON.stringify({
          ...
          deletedAnswerIds: answers
            .filter(
              (answer) => answer.toDelete && typeof answer.id === "number",
            )
  ...
  const handleInputChange =
    ...
    (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      setFormData((prev) => ({
        ...prev,
        [field]:
          field === "weight" ? parseInt(e.target.value) || 0 : e.target.value,
      }));
    };
  ...
  const handleAnswerChange =
    (
      ...
      field: keyof Omit<AnswerFormData, "id" | "isNew" | "toDelete">,
    ) =>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <Label className="text-sm font-medium">
                        Answer {index + 1}
                        {answer.isNew && (
                          <span className="text-blue-500 ml-1">(New)</span>
                        )}
                        {answer.toDelete && (
                          <span className="text-red-500 ml-1">(To Delete)</span>
                        )}
                      </Label>
                      <Button
                        type="button"
