...
export default function EvaluationsOverview({ evaluations, stats, isLoggedIn, userRole }: EvaluationsOverviewProps) {
  const router = useRouter();
  const { t } = useTranslation(['evaluation', 'property']);
  const [sortBy, setSortBy] = useState<'date' | 'score' | 'type'>('date');
  const [filterBy, setFilterBy] = useState<string>('all');
  const [sortedEvaluations, setSortedEvaluations] = useState<EvaluationData[]>(evaluations);
  const [localEvaluations, setLocalEvaluations] = useState<EvaluationData[]>(evaluations);
  
  // Dialog states
  const [editDialog, setEditDialog] = useState<{ isOpen: boolean; evaluation: EvaluationData | null }>({
    isOpen: false,
    evaluation: null,
  });
  const [deleteDialog, setDeleteDialog] = useState<{ isOpen: boolean; evaluation: EvaluationData | null }>({
    isOpen: false,
    evaluation: null,
  });
  ...
  useEffect(() => {
    ...
    if (filterBy !== 'all') {
      filtered = filtered.filter(evaluation => 
        evaluation.propertyType?.name?.toLowerCase() === filterBy.toLowerCase()
      );
    }
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'type':
          return (a.propertyType?.name || '').localeCompare(b.propertyType?.name || '');
        default:
          return 0;
      }
    });
  ...
  const handleUpdateEvaluation = async (evaluationId: number, updatedData: any) => {
    try {
        ...
        prev.map(evaluation => 
          evaluation.id === evaluationId 
            ? { 
                ...
                propertySurface: updatedData.propertySurface ? parseInt(updatedData.propertySurface) : null,
                propertyFloors: updatedData.propertyFloors,
                propertyConstructionYear: updatedData.propertyConstructionYear ? parseInt(updatedData.propertyConstructionYear) : null,
              }
            : evaluation
        )
      );
  ...
  const handleDeleteEvaluationConfirm = async (evaluationId: number) => {
    ...
    setLocalEvaluations(prev => prev.filter(evaluation => evaluation.id !== evaluationId));
  ...
  const uniquePropertyTypes = Array.from(
    new Set(localEvaluations.map(evaluation => evaluation.propertyType?.name).filter(Boolean))
  );
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-8">
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-text-muted truncate">{t('stats.totalEvaluations')}</p>
                  <p className="text-2xl font-bold text-text-base">{stats.totalEvaluations}</p>
                </div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-text-muted truncate">{t('stats.averageScore')}</p>
                  <p className="text-2xl font-bold text-text-base">{Math.round(stats.averageScore)}%</p>
                </div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-text-muted truncate">{t('stats.bestScore')}</p>
                  <p className="text-2xl font-bold text-text-base">{Math.round(stats.bestScore)}%</p>
                </div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-text-muted truncate">{t('stats.completionRate')}</p>
                  <p className="text-2xl font-bold text-text-base">{stats.completionRate}%</p>
                </div>
        ...
        {evaluations.length > 0 && (
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-6 overflow-hidden">
            <div className="flex items-center gap-2 min-w-0">
              <span className="text-sm text-text-muted flex-shrink-0">{t('filters.sortBy')}</span>
              <Select value={sortBy} onValueChange={(value: 'date' | 'score' | 'type') => setSortBy(value)}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="type">{t('filters.propertyType')}</SelectItem>
                </SelectContent>
              </Select>
            </div>

            <div className="flex items-center gap-2 min-w-0">
              <span className="text-sm text-text-muted flex-shrink-0">{t('filters.filter')}</span>
              <Select value={filterBy} onValueChange={setFilterBy}>
                ...
                <SelectContent>
                  ...
                  {uniquePropertyTypes.map(type => (
                    <SelectItem key={type} value={type?.toLowerCase() || type || 'unknown'}>
                      {t(`property.types.${type?.toLowerCase() || 'unknown'}`, type || 'Unknown')}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <div className="w-full overflow-hidden max-w-full">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 w-full max-w-full">
              {sortedEvaluations.map((evaluation, index) => (
                <Card key={evaluation.id} className="border-0 shadow-lg hover:shadow-xl transition-shadow cursor-pointer group overflow-hidden w-full max-w-full relative">
                  <CardHeader>
                    {/* Property Image Placeholder */}
                    <div className="aspect-video bg-gray-200 rounded-lg mb-4 flex items-center justify-center">
                      {getPropertyIcon(evaluation.propertyType?.name || 'unknown')}
                    </div>
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <CardTitle className="text-lg dark:text-white">
                          {evaluation.propertyName && evaluation.propertyName.trim() !== '' 
                      ...
                      <div className="flex flex-col items-end gap-2">
                        <Badge 
                          variant={evaluation.percentage >= 80 ? "default" : "secondary"}
                  ...
                  <CardContent>
                    ...
                    <div className="space-y-2 mb-4">
                      <div className="flex justify-between items-center">
                        <span className="text-sm">{t('overallScore', { ns: 'evaluation' })}</span>
                        <div className="flex items-center">
                          ...
                          <span className="text-sm font-medium w-12 text-right">{Math.round(evaluation.percentage)}/100</span>
                        </div>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm">{t('completion', { ns: 'evaluation' })}</span>
                        <div className="flex items-center">
                          <div className="w-20 bg-gray-200 rounded-full h-2 mr-2">
                            <div 
                          ...
                          <span className="text-sm font-medium w-12 text-right">{evaluation.completionRate}%</span>
                        </div>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm">{t('level', { ns: 'evaluation' })}</span>
                        <div className="flex items-center">
                          <div className="w-20 bg-gray-200 rounded-full h-2 mr-2">
                            <div 
                              className="bg-purple-600 h-2 rounded-full" 
                              style={{width: `${evaluation.level === 'Expert' ? 90 : evaluation.level === 'Good' ? 70 : 40}%`}}
                            ></div>
                          </div>
                          <span className="text-sm font-medium w-12 text-right">{evaluation.level ? t(`evaluation.levels.${evaluation.level.toLowerCase()}`, evaluation.level) : evaluation.level || 'N/A'}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 mb-4">
                        ...
                        <ul className="text-xs text-gray-600 dark:text-gray-300 space-y-1">
                          {evaluation.percentage >= 80 && (
                            <li className="flex items-center">
                              ...
                              {t('results.excellentScore', { ns: 'evaluation' })}
                            </li>
                          )}
                          {evaluation.completionRate >= 90 && (
                            <li className="flex items-center">
                              <CheckCircle className="h-3 w-3 text-green-500 mr-2 flex-shrink-0" />
                              {t('results.comprehensiveEvaluation', { ns: 'evaluation' })}
                            </li>
                          )}
                          ...
                          {evaluation.propertyLocation && (
                            <li className="flex items-center">
                              <CheckCircle className="h-3 w-3 text-green-500 mr-2 flex-shrink-0" />
                              {t('results.detailedLocation', { ns: 'evaluation' })}
                            </li>
                          )}
                        </ul>
                      </div>
                      ...
                      {(() => {
                        ...
                        return suggestions.length > 0 && (
                          <div>
                            <p className="text-sm font-medium text-red-700 mb-1">{t('results.improvements', { ns: 'evaluation' })}:</p>
                            <ul className="text-xs text-gray-600 dark:text-gray-300 space-y-1">
                              {suggestions.map((suggestion, index) => (
                                <li key={index} className="flex items-center">
                                  <X className="h-3 w-3 text-red-500 mr-2 flex-shrink-0" />
                                  {t(`results.${suggestion}`, { ns: 'evaluation' })}
                                </li>
                              ))}
                            </ul>
                          </div>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-text-muted">
                    {t('gamification.addMoreProperties', {
                      ...
                      propertyWord: evaluations.length === 4 ? t('gamification.property') : t('gamification.properties')
                    })}
                  </p>
                </div>
                <DropdownMenu>
