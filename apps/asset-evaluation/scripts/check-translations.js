...
function loadNamespaceFile(language, namespace) {
  ...
  try {
    ...
    console.error(colorize('red', `❌ Failed to load ${filePath}: ${error.message}`));
    return null;
  }
}
...
function main() {
  console.log(colorize('cyan', '🔍 Translation Keys Structure Checker (Namespace-based)'));
  ...
  for (const namespace of namespaces) {
    ...
    if (missingInRo.length > 0 || missingInEn.length > 0) {
      ...
      if (missingInRo.length > 0) {
        console.log(colorize('red', `   ❌ Missing in Romanian (${missingInRo.length}):`));
        missingInRo.forEach(key => console.log(colorize('red', `      - ${key}`)));
      }
      
      if (missingInEn.length > 0) {
        console.log(colorize('red', `   ❌ Missing in English (${missingInEn.length}):`));
        missingInEn.forEach(key => console.log(colorize('red', `      - ${key}`)));
      }
    } else {
    ...
    if (emptyEn.length > 0) {
      ...
      console.log(colorize('yellow', `   ⚠️  Empty English values (${emptyEn.length}):`));
      emptyEn.forEach(key => console.log(colorize('yellow', `      - ${key}`)));
    }
    
    if (emptyRo.length > 0) {
      totalEmptyValues += emptyRo.length;
      console.log(colorize('yellow', `   ⚠️  Empty Romanian values (${emptyRo.length}):`));
      emptyRo.forEach(key => console.log(colorize('yellow', `      - ${key}`)));
    }
  }

  console.log(colorize('cyan', '\n📋 Summary:'));
  console.log(`   📚 Namespaces checked: ${colorize('bright', namespaces.length.toString())}`);
  console.log(`   🇺🇸 Total English keys: ${colorize('bright', totalEnKeys.toString())}`);
  console.log(`   🇷🇴 Total Romanian keys: ${colorize('bright', totalRoKeys.toString())}`);

  if (allNamespacesMatch && totalEnKeys === totalRoKeys) {
    console.log(`   🔄 Structure match: ${colorize('green', 'YES')}`);
    console.log('');
    console.log(colorize('green', '✅ Perfect! All translation namespaces have identical structures.'));
    console.log(colorize('green', `✅ All ${totalEnKeys} keys are properly synchronized across all namespaces.`));
    
    if (totalEmptyValues > 0) {
      console.log(colorize('yellow', `⚠️  Found ${totalEmptyValues} empty translation values that need attention.`));
    }
    ...
    console.log(colorize('red', `❌ Mismatched namespaces: ${mismatchedNamespaces.join(', ')}`));
    console.log(colorize('red', '❌ Please fix the missing/extra keys before proceeding.'));
