...
async function checkDatabaseStatus() {
  ...
  try {
    ...
    userCount.forEach((user, index) => {
      console.log(
        `  ${index + 1}. ${user.name || "Unnamed"} (${user.email}) - ${user.role}`,
      );
    });
    ...
    teamCount.forEach((team, index) => {
      console.log(
        `  ${index + 1}. ${team.name} - ${team.planName || "No plan"} (${team.subscriptionStatus || "No status"})`,
      );
    });
    ...
    console.log(
      "   • test@test.com (password: admin123) - Original test account",
    );
    console.log(
      "   • john.doe@acme.com (password: demo123) - ACME Corporation Owner",
    );
    console.log(
      "   • sarah.wilson@acme.com (password: demo123) - ACME Corporation Admin",
    );
    console.log(
      "   • mike.johnson@acme.com (password: demo123) - ACME Corporation Member",
    );
    console.log(
      "   • emma.davis@acme.com (password: demo123) - ACME Corporation Member",
    );
    console.log(
      "   • alex.chen@startup.com (password: demo123) - Startup Inc Owner",
    );
    console.log(
