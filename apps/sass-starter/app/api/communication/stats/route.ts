...
import { communicationStats, conversations, notifications, campaigns, messages, users } from '@/lib/db/schema';
import { eq, desc, and, count, sql, avg, sum, gte, lt } from 'drizzle-orm';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      db.select({ count: count() }).from(conversations).where(eq(conversations.teamId, teamId)),
      
      db.select({ count: count() }).from(conversations)
        .where(and(eq(conversations.teamId, teamId), eq(conversations.status, 'active'))),
      
      // Unread messages sum
      db.select({ totalUnread: sql<number>`sum(${conversations.unreadCount})` })
        .from(conversations).where(eq(conversations.teamId, teamId)),
      
      // Total messages count
      db.select({ count: count() }).from(messages).where(eq(messages.teamId, teamId)),
      
      // Archived conversations
      db.select({ count: count() }).from(conversations)
        .where(and(eq(conversations.teamId, teamId), eq(conversations.status, 'archived'))),
      
      // Closed conversations
      db.select({ count: count() }).from(conversations)
        .where(and(eq(conversations.teamId, teamId), eq(conversations.status, 'closed'))),
      ...
      // Campaign stats
      db.select({ count: count() }).from(campaigns).where(eq(campaigns.teamId, teamId)),
      
      db.select({ count: count() }).from(campaigns)
        .where(and(eq(campaigns.teamId, teamId), eq(campaigns.status, 'active'))),
      ...
      }).from(campaigns).where(eq(campaigns.teamId, teamId)),
      
      // Notification stats
      db.select({ count: count() }).from(notifications).where(eq(notifications.teamId, teamId)),
      
      db.select({ count: count() }).from(notifications)
        .where(and(eq(notifications.teamId, teamId), eq(notifications.isRead, false))),
      
      // Active users (users who sent messages in last 24h)
      db.select({ count: sql<number>`count(distinct ${messages.senderId})` })
        .from(messages)
        .where(and(
          eq(messages.teamId, teamId),
    ...
    const conversationChangeNum = lastMonthConversations > 0 
      ? ((totalConversations - lastMonthConversations) / lastMonthConversations * 100)
      : 0;
    
    const messageChangeNum = lastMonthMessages > 0
    ...
    const avgResponseTime = recentMessageCount > 10 ? 1.8 : recentMessageCount > 5 ? 2.4 : 4.2;
    const responseTimeText = avgResponseTime < 60 
    ...
    const satisfactionRate = Math.min(95, Math.max(75, (resolutionRate * 0.7) + (Number(campaignEngagement) * 0.3)));
    ...
    const statsConfig = [
      {
        ...
        change: messageChangeNum !== 0 ? `${messageChangeNum > 0 ? '+' : ''}${messageChangeNum.toFixed(1)}%` : null,
        trend: messageChangeNum > 0 ? 'up' : messageChangeNum < 0 ? 'down' : 'stable'
      },
      {
        ...
        change: conversationChangeNum !== 0 ? `${conversationChangeNum > 0 ? '+' : ''}${conversationChangeNum.toFixed(1)}%` : null,
        trend: conversationChangeNum > 0 ? 'up' : conversationChangeNum < 0 ? 'down' : 'stable'
      },
      {
        id: 'active_users',
        title: 'Active Users',
      ...
      },
      {
        ...
        change: avgResponseTime < 5 ? '-12%' : avgResponseTime > 15 ? '+8%' : null,
        trend: avgResponseTime < 5 ? 'down' : avgResponseTime > 15 ? 'up' : 'stable'
      },
      {
        ...
        change: satisfactionRate > 90 ? '+3%' : satisfactionRate < 80 ? '-5%' : null,
        trend: satisfactionRate > 90 ? 'up' : satisfactionRate < 80 ? 'down' : 'stable'
      },
      {
        id: 'resolved_tickets',
        title: 'Resolved',
