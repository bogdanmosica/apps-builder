...
import { userSessions, payments, activityLogs } from '@/lib/db/schema';
  ...
  try {
    ...
    for (let dayOffset = 6; dayOffset >= 0; dayOffset--) {
      ...
      for (let sessionIndex = 0; sessionIndex < sessionsPerDay; sessionIndex++) {
        ...
        const pages = ['/', '/dashboard', '/pricing', '/settings', '/analytics'];
    ...
    if (recentActivitiesData.length > 0) {
      ...
      console.log(`✅ Created ${recentActivitiesData.length} recent activities`);
    }
    ...
    return NextResponse.json(
      { error: 'Failed to generate recent data', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
