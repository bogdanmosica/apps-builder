...
function RealTimeMetricCard({ title, value, icon: Icon, change }: RealTimeMetricCardProps) {
  return (
    <Card className='min-w-0'> {/* Ensure card can shrink */}
  ...
  return (
    <div className='flex items-center justify-between p-4 border rounded-lg'>
      ...
      <div className='text-right'>
        ...
        <p className={`text-sm flex items-center ${source.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
          {source.change >= 0 ? (
...
export default function AdvancedAnalytics() {
  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      const response = await fetch(`/api/dashboard/advanced-analytics?timeRange=${timeRange}`, {
        credentials: 'include',
      });
            ...
            <CardContent>
              <div className='space-y-4'>
                <div className='flex justify-between'>
                  ...
                  <span className='text-sm'>{analyticsData.userEngagement.totalSessions.toLocaleString()}</span>
                </div>
                <div className='flex justify-between'>
                  <span className='text-sm font-medium'>New Users</span>
                  <span className='text-sm'>{analyticsData.userEngagement.newUsers.toLocaleString()}</span>
                </div>
                <div className='flex justify-between'>
                  <span className='text-sm font-medium'>Returning Users</span>
                  <span className='text-sm'>{analyticsData.userEngagement.returningUsers.toLocaleString()}</span>
                </div>
                <div className='flex justify-between'>
                  <span className='text-sm font-medium'>Avg. Session Duration</span>
                  <span className='text-sm'>{analyticsData.userEngagement.avgSessionDuration}</span>
                </div>
                <div className='flex justify-between'>
                  <span className='text-sm font-medium'>Bounce Rate</span>
                  <span className='text-sm'>{analyticsData.userEngagement.bounceRate}%</span>
                </div>
              </div>
            </CardContent>
          </Card>
              ...
              <TableBody>
                {analyticsData.topPages.map((page) => (
                  <TableRow key={page.page}>
                    ...
                    <TableCell>
                      <Badge variant={page.bounceRate > 50 ? 'destructive' : page.bounceRate > 30 ? 'secondary' : 'default'}>
                        {page.bounceRate}%
                      </Badge>
                    </TableCell>
                  </TableRow>
                    ...
                    <div className='flex-1'>
                      <div className='flex justify-between items-center mb-2'>
                        ...
                        <span className='text-sm text-muted-foreground'>
                          {step.users.toLocaleString()} users ({step.conversion.toFixed(1)}%)
                        </span>
                      </div>
